...
--
--   - the 'Action' expects exactly as many 'ResolvedLocation' arguments
...
data Rule = Rule
  ...
  , actionId :: !ActionId
     ...
     --
     -- The 'Action' will receive exactly as many 'ResolvedLocation'
...
-- | A resolved location of a dependency or of a result of a rule, consisting
...
data Result
  ...
  -- Cabal should put it in some 'componentAutogenModules' directory.
  = AutogenFile !FilePath
  -- | Generates a file with the given relative filepath,
  -- Cabal should put it in some 'componentBuildDir' directory.
  | BuildFile !FilePath
  deriving stock ( Generic, Show )
  deriving anyclass Binary
...
--
-- The 'Action' gets passed precisely as many 'ResolvedLocation' values
-- as there are 'Dependency' values declared in the 'Rule' that calls it.
--
...
-- with the 'Result's stored in the 'Rule' that stored the 'ActionId'
-- that refers to this 'Action'.
data Action =
  Action
    { action
      :: [ ResolvedLocation ]
