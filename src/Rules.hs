...
module Rules
  ( -- * Rules
    ...
    -- ** Rule inputs/outputs
  , Dependency(..), ResultLocation(..), Result
...
data ResultLocation
  -- | A file that goes in a component autogenerated modules directory.
  = AutogenFile
  -- | A file that goes in a component build directory.
  | BuildFile
  deriving stock ( Generic, Show, Eq, Ord, Enum, Bounded )
  deriving anyclass Binary

newtype ResultDirs =
  ResultDirs { resultDir :: ResultLocation -> FilePath }
resultLocations :: [ResultLocation]
resultLocations = [ minBound .. maxBound ]
resultDirs :: ResultDirs -> [FilePath]
resultDirs (ResultDirs { resultDir = fn }) = map fn resultLocations

instance Show ResultDirs where
  show dirs = show $ resultDirs dirs
instance Eq ResultDirs where
  f == g = resultDirs f == resultDirs g
instance Ord ResultDirs where
  compare f g = compare (resultDirs f) (resultDirs g)
instance Binary ResultDirs where
  put f = mapM_ Binary.put $ resultDirs f
  get = do
    -- NB: be careful to ensure that the order here
    -- matches the Enum instance for 'ResultLocation'
    autogenDir <- Binary.get
    buildDir <- Binary.get
    return $ ResultDirs $ \ loc -> case loc of
      AutogenFile -> autogenDir
      BuildFile -> buildDir
